...
func (w *WSURL) GetURL() string {
	url_ := url.URL{Scheme: w.Scheme, Host: w.Host, Path: w.BaseURN}

	return url_.String()
}

func (w *WSURL) GetURLWithMethod(method string) string {
	url_ := url.URL{
		Scheme: w.Scheme,
		Host:   w.Host,
		Path:   w.BaseURN + method,
	}
